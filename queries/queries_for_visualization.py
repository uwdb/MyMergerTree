...
store = "store(edges," + table_prefix + "edgesTableFinal);"
...
f.write("{" + '\n');
#--nodes
current_schema = (MyriaRelation(relation= edgesTableFinal, connection=connection).schema.to_dict())
columnNames = [x.encode('utf-8') for x in current_schema['columnNames']]
columnTypes = [x.encode('utf-8') for x in current_schema['columnTypes']]
columns = zip(columnNames, columnTypes)
f.write("'" + edgesTableFinal + "' : " +  str(columns) + ',\n');
#--edges
...
current_query = "sortByChildMass = [from scan("+ edgesTableFinal + ") as e, scan(" + haloTableProg+ ") as h where timeStep = currentTime + 1 and h.grpID = e.nextGroup and h.nowGroup = e.nowGroup emit nowGroup, currentTime, currentGroup, nextGroup, mass as nextGroupMass]; store(sortByChildMass," + table_prefix + "edgesWithMassSort);"
...
maxMasses1 = "maxMasses1 = [from bothMaxGroups g, haloTable h where h.nowGroup = g.nowGroup and h.grpID = g.e1NextGroup and h.timeStep = g.currentTime+1 emit h.nowGroup, h.grpID, h.timeStep, h.mass, h.totalParticles, h.prog, g.massRatio*1.0 as massRatio];"
maxMasses2 = "maxMasses2 = [from bothMaxGroups g, haloTable h where h.nowGroup = g.nowGroup and h.grpID = g.e2NextGroup and h.timeStep = g.currentTime+1 emit h.nowGroup, h.grpID, h.timeStep, h.mass, h.totalParticles, h.prog, g.massRatio*1.0 as massRatio];"
maxMasses = "maxMasses = maxMasses1 + maxMasses2;"
remainingHalos = "remainingHalos = diff([from haloTable h emit h.nowGroup, h.grpID, h.timeStep, h.mass, h.totalParticles, h.prog], [from maxMasses h emit h.nowGroup, h.grpID, h.timeStep, h.mass, h.totalParticles, h.prog]);"
remainingHalosMass = "remainingHalosMass = [from remainingHalos h emit h.nowGroup, h.grpID, h.timeStep, h.mass, h.totalParticles, h.prog, -1+0.0 as massRatio];"
